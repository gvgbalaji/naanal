...
from openstack_dashboard.dashboards.compute.images \
...
class SelectComputeUserAction(workflows.Action):
    compute_id = forms.ChoiceField(label=_("Compute"))
    user_id = forms.ChoiceField(label=_("User"))

    def __init__(self, request, *args, **kwargs):
        super(SelectComputeUserAction, self).__init__(request, *args, **kwargs)
        # Set our compute choices
        computes = [(tenant.id, tenant.name)
                    for tenant in request.user.authorized_tenants]
        self.fields['compute_id'].choices = computes
...
class SelectComputeUser(workflows.Step):
    action_class = SelectComputeUserAction
    contributes = ("compute_id", "user_id")
...
class SetInstanceDetailsAction(workflows.Action):
    ...
    class Meta(object):
        name = _("Details")
        help_text_template = ("compute/instances/"
    ...
    def get_help_text(self, extra_context=None):
        ...
        try:
            ...
            images = image_utils.get_available_images(
                self.request, self.initial['compute_id'], self._images_cache)
...
KEYPAIR_IMPORT_URL = "horizon:networking:basics:keypairs:import"
...
class SetNetwork(workflows.Step):
    ...
    if api.neutron.is_port_profiles_supported():
        ...
        template_name = "compute/instances/_update_networks.html"
...
class LaunchInstance(workflows.Workflow):
    ...
    success_url = "horizon:compute:instances:index"
    multipart = True
    default_steps = (SelectComputeUser,
                     SetInstanceDetails,
                     SetAccessControls,
                     SetNetwork,
                     PostCreationStep,
