...
class AllocateIP(tables.LinkAction):
    name = "allocate"
    verbose_name = _("Allocate IP To Compute")
    classes = ("ajax-modal",)
    icon = "link"
    url = "horizon:networking:basics:floating_ips:allocate"

    def single(self, data_table, request, *args):
        return shortcuts.redirect('horizon:networking:basics:index')
    ...
    def allowed(self, request, fip=None):
        ...
        if usages['floating_ips']['available'] <= 0:
            ...
            self.verbose_name = _("Allocate IP To Compute")
        ...
        if api.base.is_service_enabled(request, "network"):
            ...
            policy = (("network", "network_extension:floating_ips"),
                      ("network", "network:allocate_floating_ip"),)
...
class ReleaseIPs(tables.BatchAction):
    ...
    def allowed(self, request, fip=None):
        if api.base.is_service_enabled(request, "network"):
            ...
            policy = (("network", "network_extension:floating_ips"),
                      ("network", "network:release_floating_ip"),)
...
class AssociateIP(tables.LinkAction):
    ...
    def allowed(self, request, fip):
        if api.base.is_service_enabled(request, "network"):
            ...
            policy = (("network", "network_extension:floating_ips"),
                      ("network", "network:associate_floating_ip"),)
...
class DisassociateIP(tables.Action):
    ...
    def allowed(self, request, fip):
        if api.base.is_service_enabled(request, "network"):
            ...
            policy = (("network", "network_extension:floating_ips"),
                      ("network", "network:disassociate_floating_ip"),)
...
def get_instance_link(datum):
    if datum.instance_type == 'network':
        return reverse("horizon:network:instances:detail",
