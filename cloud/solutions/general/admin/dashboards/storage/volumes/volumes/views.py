...
from openstack_dashboard.dashboards.storage.volumes \
    .volumes import forms as storage_forms

from openstack_dashboard.dashboards.storage.volumes \
    .volumes import tables as storage_tables
from openstack_dashboard.dashboards.storage.volumes \
    .volumes import tabs as storage_tabs


class DetailView(tabs.TabView):
    tab_group_class = storage_tabs.VolumeDetailTabs
    template_name = 'storage/volumes/volumes/detail.html'
    page_title = _("Volume Details: {{ volume.name }}")

    def get_context_data(self, **kwargs):
        context = super(DetailView, self).get_context_data(**kwargs)
        volume = self.get_data()
        table = storage_tables.VolumesTable(self.request)
...
class CreateView(forms.ModalFormView):
    form_class = storage_forms.CreateForm
    modal_header = _("Create Volume")
    template_name = 'storage/volumes/volumes/create.html'
    submit_label = _("Create Volume")
    submit_url = reverse_lazy("horizon:storage:volumes:volumes:create")
    success_url = reverse_lazy('horizon:storage:volumes:volumes_tab')
...
class ExtendView(forms.ModalFormView):
    form_class = storage_forms.ExtendForm
    modal_header = _("Extend Volume")
    template_name = 'storage/volumes/volumes/extend.html'
    submit_label = _("Extend Volume")
    submit_url = "horizon:storage:volumes:volumes:extend"
    success_url = reverse_lazy("horizon:storage:volumes:index")
...
class CreateSnapshotView(forms.ModalFormView):
    form_class = storage_forms.CreateSnapshotForm
    modal_header = _("Create Volume Snapshot")
    template_name = 'storage/volumes/volumes/create_snapshot.html'
    submit_url = "horizon:storage:volumes:volumes:create_snapshot"
    success_url = reverse_lazy('horizon:storage:volumes:snapshots_tab')
...
class UploadToImageView(forms.ModalFormView):
    form_class = storage_forms.UploadToImageForm
    modal_header = _("Upload Volume to Image")
    template_name = 'storage/volumes/volumes/upload_to_image.html'
    submit_label = _("Upload")
    submit_url = "horizon:storage:volumes:volumes:upload_to_image"
    success_url = reverse_lazy("horizon:storage:volumes:index")
...
class CreateTransferView(forms.ModalFormView):
    form_class = storage_forms.CreateTransferForm
    template_name = 'storage/volumes/volumes/create_transfer.html'
    success_url = reverse_lazy('horizon:storage:volumes:volumes_tab')
    modal_id = "create_volume_transfer_modal"
    modal_header = _("Create Volume Transfer")
    submit_label = _("Create Volume Transfer")
    submit_url = "horizon:storage:volumes:volumes:create_transfer"
...
class AcceptTransferView(forms.ModalFormView):
    form_class = storage_forms.AcceptTransferForm
    template_name = 'storage/volumes/volumes/accept_transfer.html'
    success_url = reverse_lazy('horizon:storage:volumes:volumes_tab')
    modal_id = "accept_volume_transfer_modal"
    modal_header = _("Accept Volume Transfer")
    submit_label = _("Accept Volume Transfer")
    submit_url = reverse_lazy(
        "horizon:storage:volumes:volumes:accept_transfer")
    page_title = _("Accept Volume Transfer")


class ShowTransferView(forms.ModalFormView):
    form_class = storage_forms.ShowTransferForm
    template_name = 'storage/volumes/volumes/show_transfer.html'
    success_url = reverse_lazy('horizon:storage:volumes:volumes_tab')
    modal_id = "show_volume_transfer_modal"
    modal_header = _("Volume Transfer")
    submit_url = "horizon:storage:volumes:volumes:show_transfer"
...
class UpdateView(forms.ModalFormView):
    form_class = storage_forms.UpdateForm
    modal_header = _("Edit Volume")
    modal_id = "update_volume_modal"
    template_name = 'storage/volumes/volumes/update.html'
    submit_url = "horizon:storage:volumes:volumes:update"
    success_url = reverse_lazy("horizon:storage:volumes:index")
    ...
    def get_object(self):
        if not hasattr(self, "_object"):
            ...
            try:
                ...
                url = reverse('horizon:storage:volumes:index')
...
class EditAttachmentsView(tables.DataTableView, forms.ModalFormView):
    table_class = storage_tables.AttachmentsTable
    form_class = storage_forms.AttachForm
    form_id = "attach_volume_form"
    modal_header = _("Manage Volume Attachments")
    modal_id = "attach_volume_modal"
    template_name = 'storage/volumes/volumes/attach.html'
    submit_url = "horizon:storage:volumes:volumes:attach"
    success_url = reverse_lazy("horizon:storage:volumes:index")
...
class RetypeView(forms.ModalFormView):
    form_class = storage_forms.RetypeForm
    modal_id = "retype_volume_modal"
    modal_header = _("Change Volume Type")
    template_name = 'storage/volumes/volumes/retype.html'
    submit_label = _("Change Volume Type")
    submit_url = "horizon:storage:volumes:volumes:retype"
    success_url = reverse_lazy("horizon:storage:volumes:index")
...
class EncryptionDetailView(generic.TemplateView):
    template_name = 'storage/volumes/volumes/encryption_detail.html'
