...
from openstack_dashboard.dashboards.overview.network_topology.instances \
    import tables as instances_tables
from openstack_dashboard.dashboards.overview.network_topology.ports \
    import tables as ports_tables
from openstack_dashboard.dashboards.overview.network_topology.routers \
    import tables as routers_tables

from openstack_dashboard.dashboards.compute.instances import\
    console as i_console
from openstack_dashboard.dashboards.compute.instances import\
    views as i_views
from openstack_dashboard.dashboards.compute.instances.workflows import\
    create_instance as i_workflows
from openstack_dashboard.dashboards.networking.basics.networks import\
    views as n_views
from openstack_dashboard.dashboards.networking.basics.networks import\
    workflows as n_workflows
from openstack_dashboard.dashboards.networking.routers import\
    views as r_views


class NTCreateRouterView(r_views.CreateView):
    template_name = 'overview/network_topology/create_router.html'
    success_url = reverse_lazy("horizon:overview:network_topology:index")
    page_title = _("Create a Router")


class NTCreateNetwork(n_workflows.CreateNetwork):
    def get_success_url(self):
        return reverse("horizon:overview:network_topology:index")
...
class NTLaunchInstance(i_workflows.LaunchInstance):
    success_url = "horizon:overview:network_topology:index"
...
class InstanceView(i_views.IndexView):
    ...
    template_name = 'overview/network_topology/iframe.html'
...
class NetworkTopologyView(views.HorizonTemplateView):
    template_name = 'overview/network_topology/index.html'
...
class JSONView(View):
    ...
    def _get_servers(self, request):
        ...
        self.add_resource_url('horizon:compute:instances:detail', data)
        return data
    ...
    def _get_networks(self, request):
        ...
        self.add_resource_url('horizon:networking:basics:networks:detail',
                              networks)
    ...
    def _get_routers(self, request):
        ...
        self.add_resource_url('horizon:networking:routers:detail', routers)
        return routers
    ...
    def _get_ports(self, request):
        ...
        self.add_resource_url('horizon:networking:basics:networks:ports:detail',
                              ports)
        return ports
