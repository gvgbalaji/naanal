...
def get_available_images(request, compute_id=None, images_cache=None):
    """Returns a list of images that are public or owned by the given
    compute_id. If compute_id is not specified, only public images
    are returned.

    :param images_cache: An optional dict-like object in which to
     cache public and per-compute id image metadata.

    """
    if images_cache is None:
        images_cache = {}
    public_images = images_cache.get('public_images', [])
    images_by_compute = images_cache.get('images_by_compute', {})
    ...
    # Preempt if we don't have a compute_id yet.
    if compute_id is None:
        images_by_compute[compute_id] = []

    if compute_id not in images_by_compute:
        owner = {"property-owner_id": compute_id,
                 "status": "active"}
        try:
            owned_images, _more, _prev = glance.image_list_detailed(
                request, filters=owner)
            images_by_compute[compute_id] = owned_images
        except Exception:
            owned_images = []
            exceptions.handle(request,
        ...
        owned_images = images_by_compute[compute_id]

    if 'images_by_compute' not in images_cache:
        images_cache['images_by_compute'] = images_by_compute
...
def image_field_data(request, include_empty_option=False):
    ...
    """
    try:
        images = get_available_images(request, request.user.compute_id)
    except Exception:
