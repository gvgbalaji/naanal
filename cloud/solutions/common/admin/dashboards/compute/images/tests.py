...
from openstack_dashboard.dashboards.compute.images import utils
from openstack_dashboard.test import helpers as test


INDEX_URL = reverse('horizon:compute:images:index')
CREATE_URL = reverse('horizon:compute:images:images:create')
...
class ImagesAndSnapshotsTests(test.TestCase):
    ...
    def test_index(self):
        ...
        self.assertTemplateUsed(res, 'compute/images/index.html')
...
class ImagesAndSnapshotsUtilsTests(test.TestCase):
    ...
    def test_list_image_using_cache(self):
        ...
        self.assertEqual(1, len(images_cache['images_by_compute']))
        self.assertEqual(
            len(private_images),
            len(images_cache['images_by_compute'][self.tenant.id]))
        ...
        self.assertEqual(
            len(public_images),
            len(images_cache['public_images']))
        self.assertEqual(2, len(images_cache['images_by_compute']))
        self.assertEqual(
            len(private_images),
            len(images_cache['images_by_compute']['other-tenant']))
    ...
    def test_list_image_error_private_image_list(self):
        ...
        api.glance.image_list_detailed(
            ...
            .AndRaise(self.exceptions.glance)
        exceptions.handle(IsA(http.HttpRequest),
                          "Unable to retrieve images for the current compute.")
        api.glance.image_list_detailed(
            IsA(http.HttpRequest),
            filters={'property-owner_id': self.tenant.id,
                     'status': 'active'}) \
        ...
        self.assertEqual(
            len(public_images),
            len(images_cache['public_images']))
        self.assertFalse(len(images_cache['images_by_compute']))
