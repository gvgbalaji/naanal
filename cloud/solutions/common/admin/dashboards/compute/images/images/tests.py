...
from openstack_dashboard.dashboards.compute.images.images import forms
from openstack_dashboard.dashboards.compute.images.images import tables


IMAGES_INDEX_URL = reverse('horizon:compute:images:index')
...
class UpdateImageFormTests(test.TestCase):
    ...
    def test_image_update(self):
        ...
        url = reverse('horizon:compute:images:images:update',
                      args=[image.id])
...
class ImageViewTests(test.TestCase):
    def test_image_create_get(self):
        url = reverse('horizon:compute:images:images:create')
        res = self.client.get(url)
        self.assertTemplateUsed(res,
                                'compute/images/images/create.html')
    ...
    def test_image_detail_get(self):
        ...
        res = self.client.get(reverse('horizon:compute:images:images:detail',
                                      args=[image.id]))

        self.assertTemplateUsed(res,
                                'compute/images/images/detail.html')
    ...
    def test_protected_image_detail_get(self):
        ...
        res = self.client.get(
            reverse('horizon:compute:images:images:detail',
                    args=[image.id]))
        self.assertTemplateUsed(res,
                                'compute/images/images/detail.html')
        self.assertEqual(res.context['image'].protected, image.protected)
    ...
    def test_image_detail_get_with_exception(self):
        ...
        url = reverse('horizon:compute:images:images:detail',
                      args=[image.id])
        res = self.client.get(url)
    ...
    def test_image_update_get(self):
        ...
        res = self.client.get(
            reverse('horizon:compute:images:images:update',
                    args=[image.id]))

        self.assertTemplateUsed(res,
                                'compute/images/images/_update.html')
        self.assertEqual(res.context['image'].name, image.name)
...
class OwnerFilterTests(test.TestCase):
    ...
    def test_filter(self):
        ...
        images = filter_.filter(table, all_images, 'compute')
        self.assertEqual(images, self._expected('compute'))
