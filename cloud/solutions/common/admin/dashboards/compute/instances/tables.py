...
from openstack_dashboard.dashboards.networking.basics.floating_ips \
    import workflows
from openstack_dashboard.dashboards.compute.instances import tabs
from openstack_dashboard.dashboards.compute.instances.workflows \
    ...
    def allowed(self, request, instance=None):
        ...
        if policy_check:
            has_permission = policy_check(
                policy, request,
                target={'compute_id': getattr(instance, 'tenant_id', None)})
...
class LaunchLink(tables.LinkAction):
    ...
    url = "horizon:compute:instances:launch"
...
class EditInstance(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:compute:instances:update"
    classes = ("ajax-modal",)
    icon = "pencil"
    policy_rules = (("compute", "compute:update"),)

    def get_link_url(self, compute):
        return self._get_link_url(compute, 'instance_info')

    def _get_link_url(self, compute, step_slug):
        base_url = urlresolvers.reverse(self.url, args=[compute.id])
...
class EditInstanceSecurityGroups(EditInstance):
    ...
    def get_link_url(self, compute):
        return self._get_link_url(compute, 'update_security_groups')
...
class CreateSnapshot(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:compute:images:snapshots:create"
    classes = ("ajax-modal",)
    icon = "camera"
...
class ConsoleLink(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:compute:instances:detail"
    classes = ("btn-console",)
...
class ResizeLink(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:compute:instances:resize"
    classes = ("ajax-modal", "btn-resize")
    policy_rules = (("compute", "compute:resize"),)

    def get_link_url(self, compute):
        return self._get_link_url(compute, 'flavor_choice')
...
class RebuildInstance(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:compute:instances:rebuild"
...
class DecryptInstancePassword(tables.LinkAction):
    ...
    url = "horizon:compute:instances:decryptpassword"
...
class AssociateIP(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:networking:basics:floating_ips:associate"
    classes = ("ajax-modal",)
    icon = "link"
...
def get_size(instance):
    if hasattr(instance, "full_flavor"):
        template_name = 'compute/instances/_instance_flavor.html'
...
class InstancesTable(tables.DataTable):
    ...
    name = tables.Column("name",
                         link="horizon:compute:instances:detail",
