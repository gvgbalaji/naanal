...
from openstack_dashboard.dashboards.compute.instances \
    import console as compute_console
from openstack_dashboard.dashboards.compute.instances \
    import forms as compute_forms
from openstack_dashboard.dashboards.compute.instances \
    import tables as compute_tables
from openstack_dashboard.dashboards.compute.instances \
    import tabs as compute_tabs
from openstack_dashboard.dashboards.compute.instances \
    import workflows as compute_workflows

LOG = logging.getLogger(__name__)


class IndexView(tables.DataTableView):
    table_class = compute_tables.InstancesTable
    template_name = 'compute/instances/index.html'
    ...
    def get_data(self):
        marker = self.request.GET.get(
            compute_tables.InstancesTable._meta.pagination_param, None)
...
class LaunchInstanceView(workflows.WorkflowView):
    workflow_class = compute_workflows.LaunchInstance

    def get_initial(self):
        initial = super(LaunchInstanceView, self).get_initial()
        initial['compute_id'] = self.request.user.tenant_id
...
def vnc(request, instance_id):
    try:
        ...
        console_url = compute_console.get_console(request, 'VNC', instance)[1]
...
def spice(request, instance_id):
    try:
        instance = api.nova.server_get(request, instance_id)
        console_url = compute_console.get_console(request, 'SPICE',
                                                  instance)[1]
        return shortcuts.redirect(console_url)
    except Exception:
        redirect = reverse("horizon:compute:instances:index")
...
def rdp(request, instance_id):
    try:
        instance = api.nova.server_get(request, instance_id)
        console_url = compute_console.get_console(request, 'RDP', instance)[1]
        return shortcuts.redirect(console_url)
    except Exception:
        redirect = reverse("horizon:compute:instances:index")
...
class SerialConsoleView(generic.TemplateView):
    template_name = 'compute/instances/serial_console.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            console_url = compute_console.get_console(self.request,
                                                      "SERIAL", instance)[1]
...
class UpdateView(workflows.WorkflowView):
    workflow_class = compute_workflows.UpdateInstance
    success_url = reverse_lazy("horizon:compute:instances:index")
...
class RebuildView(forms.ModalFormView):
    form_class = compute_forms.RebuildInstanceForm
    template_name = 'compute/instances/rebuild.html'
    success_url = reverse_lazy('horizon:compute:instances:index')
...
class DecryptPasswordView(forms.ModalFormView):
    form_class = compute_forms.DecryptPasswordInstanceForm
    template_name = 'compute/instances/decryptpassword.html'
    success_url = reverse_lazy('horizon:compute:instances:index')
...
class DetailView(tabs.TabView):
    tab_group_class = compute_tabs.InstanceDetailTabs
    template_name = 'compute/instances/detail.html'
    redirect_url = 'horizon:compute:instances:index'
    ...
    def get_context_data(self, **kwargs):
        ...
        table = compute_tables.InstancesTable(self.request)
    ...
    def get_data(self):
        ...
        status_label = [label for (value, label) in
                        compute_tables.STATUS_DISPLAY_CHOICES
...
class ResizeView(workflows.WorkflowView):
    workflow_class = compute_workflows.ResizeInstance
    success_url = reverse_lazy("horizon:compute:instances:index")
