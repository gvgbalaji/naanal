...
class DeleteRouter(policy.PolicyTargetMixin, tables.DeleteAction):
    ...
    redirect_url = "horizon:network:routers:index"
...
class CreateRouter(tables.LinkAction):
    ...
    url = "horizon:network:routers:create"
    classes = ("ajax-modal",)
    icon = "plus"
...
class EditRouter(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:network:routers:update"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class SetGateway(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:network:routers:setgateway"
    classes = ("ajax-modal",)
    icon = "camera"
    policy_rules = (("network", "update_router"),)
...
class RoutersTable(tables.DataTable):
    ...
    name = tables.Column("name",
                         verbose_name=_("Name"),
                         link="horizon:network:routers:detail")
