...
class VolumePolicyTargetMixin(policy.PolicyTargetMixin):
    policy_target_attrs = (("storage_id", 'os-vol-tenant-attr:tenant_id'),)


class LaunchVolume(tables.LinkAction):
    name = "launch_volume"
    verbose_name = _("Launch as Instance")
    url = "horizon:storage:instances:launch"
...
class ExtendVolume(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:storage:volumes:volumes:extend"
...
class EditAttachments(tables.LinkAction):
    ...
    url = "horizon:storage:volumes:volumes:attach"
    classes = ("ajax-modal",)
    icon = "pencil"

    def allowed(self, request, volume=None):
        if volume:
            storage_id = getattr(volume, "os-vol-tenant-attr:tenant_id", None)
            attach_allowed = \
...
class CreateSnapshot(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:storage:volumes:volumes:create_snapshot"
    classes = ("ajax-modal",)
    icon = "camera"
...
class CreateTransfer(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:storage:volumes:volumes:create_transfer"
    classes = ("ajax-modal",)
...
class CreateBackup(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:storage:volumes:volumes:create_backup"
    classes = ("ajax-modal",)
...
class UploadToImage(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:storage:volumes:volumes:upload_to_image"
    classes = ("ajax-modal",)
    icon = "cloud-upload"
...
class EditVolume(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:storage:volumes:volumes:update"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class RetypeVolume(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:storage:volumes:volumes:retype"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class AcceptTransfer(tables.LinkAction):
    ...
    url = "horizon:storage:volumes:volumes:accept_transfer"
    classes = ("ajax-modal",)
    icon = "exchange"
...
def get_attachment_name(request, attachment):
    ...
    try:
        url = reverse("horizon:storage:instances:detail", args=(server_id,))
