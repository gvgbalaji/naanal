...
INDEX_URL = reverse('horizon:storage:volumes:index')
VOLUME_SNAPSHOTS_TAB_URL = reverse('horizon:storage:volumes:snapshots_tab')
...
class VolumeSnapshotsViewTests(test.TestCase):
    ...
    def test_create_snapshot_get(self):
        ...
        url = reverse('horizon:storage:volumes:'
                      'volumes:create_snapshot', args=[volume.id])
        res = self.client.get(url)

        self.assertTemplateUsed(res, 'storage/volumes/volumes/'
    ...
    def test_create_snapshot_post(self):
        ...
        url = reverse('horizon:storage:volumes:volumes:create_snapshot',
                      args=[volume.id])
    ...
    def test_volume_snapshot_detail_get(self):
        ...
        url = reverse('horizon:storage:volumes:snapshots:detail',
                      args=[snapshot.id])
        res = self.client.get(url)
    ...
    def test_update_snapshot(self):
        ...
        formData = {'method': 'UpdateSnapshotForm',
                    'name': snapshot.name,
                    'description': snapshot.description}
        url = reverse(('horizon:storage:volumes:snapshots:update'),
                      args=[snapshot.id])
        res = self.client.post(url, formData)
        self.assertRedirectsNoFollow(res, INDEX_URL)
